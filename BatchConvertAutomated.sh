#!/bin/bash
# MADE BY Chris aka cjwsam ON GIT 
# Adjust as needed.
SEARCH_LOCATION="LOCATION OF UR FILES"
LOG_FILE=""
MIN_SIZE=30M #WILL PREVENT SMALL METADATA FILES FROM BEING READ
FILE_EXTENSIONS=(
    "mp4"
    "avi"
    "m4v"
...
for filename in "${process_movies[@]}"; do :
  ...
  ConvertAutomated.sh "$filename"         #THIS IS IMPORTANT MAKE SURE IT LINKS TO THE OTHER FILE
    exitCode=$?
...
su plex -c "/usr/lib/plexmediaserver/Plex\ Media\ Scanner --scan"  #WILL EXECURE A PLEX UPDATE UPON COMPLETION 
echo ""
